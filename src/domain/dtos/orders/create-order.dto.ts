...
export class CreateOrderDto {
    ...
    static create(object: { [key: string]: any }): [string[]?, CreateOrderDto?] {
      const { negocio_id, user_id, status, subtotal, tax, total, cartItems=[]} = object;
      const errors: string[] = [];

      if (negocio_id === undefined || negocio_id === null) {
        errors.push('negocio_id is required');
      } else if (!Number.isInteger(negocio_id) || negocio_id <= 0) {
        errors.push('negocio_id must be a positive integer');
      }
      ...
      if (!Array.isArray(cartItems) || cartItems.length === 0) {
          errors.push('cartItems must not be empty. [{price:number, product_id:number, quantity:number}]');
      }
      ...
      return [[], new CreateOrderDto(negocio_id, user_id, status, cartItems)];
    }
  }
